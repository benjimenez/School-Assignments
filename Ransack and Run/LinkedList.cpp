...
bool LinkedList::Find(const int n){
  ...
  LinkedListElement* temp = m_pHead;
  if(temp == NULL)return false; //list is empty
...
void LinkedList::Insert(const int n){
  ...
  m_pHead = new LinkedListElement(n); //create new node
...
bool LinkedList::isEmpty(){
  ...
  if(m_pHead == NULL)
  return true; //bail if the list is empty
  else
...
/// \param n [OUT] Item stored at list element indicated by iterator.
/// \return true for success.

bool LinkedList::Get(int& n){
  if(m_pCurrent != NULL){
    n = m_pCurrent->m_nData;
    return true;
  } //if
  else return false;
} //Get

/// \return true if iterator is at the last item in the list.

bool LinkedList::Last(){
  if(m_pCurrent != NULL){
    return m_pCurrent->m_pNext == NULL;
  } //if
  else return false;
} //Last
